...
id: installation
title: Installation
description: A command line tool, you can write the command line tool like a koa. Slowly's goal is simply to get you more focused on writing business code and leave the rest to slowly
---

A command line tool, you can write the command line tool like a koa. Slowly's goal is simply to get you more focused on writing business code and leave the rest to slowly
...
1.  Ensure you have the latest version of [Node](https://nodejs.org/en/download/) installed.
...
1.  Create a project, if none exists, and change your directory to this project's root.
    ...
    ```
    $ mkdir slowly-example && cd slowly-example
    ```

1.  Run the slowly installation script: `npx slowly-init`.

    > If you don't have Node 8.2+ or if you prefer to install Docusaurus globally, run `yarn global add slowly-init` or `npm install --global slowly-init`. After that, run `slowly-init`.

1. Then get up and run by using the following commands.
